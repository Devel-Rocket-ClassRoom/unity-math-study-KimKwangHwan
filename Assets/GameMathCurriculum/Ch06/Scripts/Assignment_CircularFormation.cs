...
public class Assignment_CircularFormation : MonoBehaviour
{
    ...
    private float accumulateSpeed = 0f;
    private void Update()
    {
        ...
        float angle = 360f / units.Length;
        accumulateSpeed += (angleSpeed * Time.deltaTime);
        for (int i = 0; i < units.Length; i++)
        {
            if (units[i] == null) continue;
            // TODO
            Quaternion rotation = Quaternion.AngleAxis(angle * i + accumulateSpeed, leader.transform.up);
            Vector3 offset = rotation * (leader.transform.forward * formationRadius);
            units[i].transform.position = leader.transform.position + offset;
            units[i].rotation = rotation * leader.rotation;
            //Quaternion look = Quaternion.LookRotation(units[i].transform.position - leader.transform.position);
            //units[i].transform.rotation = look;
        }
    ...
    private void OnDrawGizmos()
    {
        ...
        Vector3 start = leader.transform.forward * formationRadius + leader.transform.position;
        int segments = 128;

        for (int j = 1; j < segments; j++)
        {
            Quaternion rotation = Quaternion.AngleAxis(360f / segments * j, leader.transform.up);
            Vector3 offset = rotation * (leader.transform.forward * formationRadius);
            Vector3 next = leader.transform.position + offset;
