...
public class Assignment_EnemyDetector : MonoBehaviour
{
    ...
    private bool IsDetected(Transform enemy)
    {
        ...
        Vector3 directionNorm = targetDirection.normalized;
        float dotValue = Vector3.Dot(directionNorm, transform.forward);
        float angle = Mathf.Acos(dotValue) * Mathf.Rad2Deg;

        return targetDistance < detectionRange && angle < detectionFOV * 0.5f;
    }
