...
public class Assignment_PlanetOrbit : MonoBehaviour
{
    ...
    private void Start()
    {
        transform.position = orbitCenter + new Vector3(planetOrbitRadius, 0f, 0f);
        satellite.position = transform.position + new Vector3(satelliteOrbitRadius, 0f, 0f);
    }
    private float planetCurrentAngle = 0f;
    private float satelliteCurrentAngle = 0f;
    private void Update()
    {
        // TODO

        planetCurrentAngle += planetOrbitSpeed * Time.deltaTime;
        planetCurrentAngle %= 360f;
        Matrix4x4 planetMat = Matrix4x4.TRS(
            orbitCenter,
            Quaternion.Euler(0f, planetCurrentAngle, 0f),
            Vector3.one
        );
        transform.position = planetMat.MultiplyPoint(new Vector3(planetOrbitRadius, 0f, 0f));

        satelliteCurrentAngle += satelliteOrbitSpeed * Time.deltaTime;
        satelliteCurrentAngle %= 360f;
        Matrix4x4 satMat = Matrix4x4.TRS(
            transform.position,
            Quaternion.Euler(0f, satelliteCurrentAngle, 0f),
            Vector3.one
        );
        satellite.position = satMat.MultiplyPoint(new Vector3(satelliteOrbitRadius, 0f, 0f));
